...
interface ContactDrawerProps {
  ...
  isLoading: boolean;
  phone: string; // Telefone da conversa (caso nao tenha contato no banco ainda)
  name: string;  // Nome da conversa
  onSyncMissingContact: (phone: string) => Promise<void>;
}

export const ContactDrawer: React.FC<ContactDrawerProps> = ({ contact, isLoading, phone, name, onSyncMissingContact }) => {
    ...
    return (
      <div className="w-80 bg-af-black border-l border-gray-800 p-6 h-full overflow-y-auto">
        ...
        <div className="bg-[#1E2028] p-4 rounded-xl border border-gray-700 text-center">
          <p className="text-gray-400 text-sm">Este contato ainda nao possui ficha cadastrada no CRM.</p>
          <button
            onClick={() => onSyncMissingContact(phone)}
            className="mt-4 text-xs bg-gray-700 hover:bg-gray-600 text-white px-3 py-2 rounded transition-colors w-full"
          >
            Sincronizar Dados
          </button>
        </div>
  ...
  return (
    <div className="w-80 bg-af-black border-l border-gray-800 h-full overflow-y-auto custom-scrollbar flex flex-col">
      <div className="p-6 border-b border-gray-800 text-center relative overflow-hidden">
        <div
          className={`absolute top-0 left-0 w-full h-full opacity-5 pointer-events-none ${(contact.temperatura || '').toLowerCase().includes('quente') ? 'bg-red-600' : 'bg-blue-600'}`}
        />
        ...
        {contact.ultima_interacao && (
          <div className="pt-4 border-t border-gray-800">
            <div className="flex items-center gap-2 text-xs text-gray-500">
              <Clock size={12} />
              <span>Ultima interacao: {new Date(contact.ultima_interacao).toLocaleDateString()}</span>
            </div>
          </div>
        )}
      </div>
